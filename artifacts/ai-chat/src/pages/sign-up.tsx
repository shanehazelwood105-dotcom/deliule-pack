...
export default function SignUp() {
  ...
  const handleSocial = async (strategy: "oauth_google" | "oauth_apple" | "oauth_github") => {
    ...
    try {
      ...
      const clerkErr = err as { errors?: { message: string }[] };
      setError(clerkErr.errors?.[0]?.message ?? "Social sign up failed.");
          ...
          <div className="flex flex-col items-center mb-8">
            ...
            {step === "form" ? (
              <>
                ...
                <p className="text-zinc-400 text-sm mt-1">Join Deliule today</p>
              </>
            ) : (
              <>
            ...
            <form onSubmit={handleVerify} className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="123456"
                  maxLength={6}
                  className="w-full px-4 py-2.5 rounded-xl bg-zinc-800 border border-zinc-700 text-zinc-100 placeholder-zinc-500 text-sm text-center tracking-widest font-mono focus:outline-none focus:ring-2 focus:ring-violet-500/40 focus:border-violet-500/60 transition-all"
                />
              </div>
              <button
                type="submit"
                disabled={loading || !isLoaded}
                className="w-full py-2.5 rounded-xl bg-gradient-premium text-white font-medium text-sm hover:opacity-90 transition-opacity disabled:opacity-50 flex items-center justify-center gap-2"
              >
                {loading && <Loader2 className="w-4 h-4 animate-spin" />}
                Verify email
              </button>
